...
// LOCAL MODIFICATION (fork of alshedivat/al-folio).
//
// The thin-starter boundary below applies to the UPSTREAM starter repo, not to
// a site built from it: docs/ARCHITECTURE.md ("Local overrides: your site vs.
// this repo") states that a user's own site may legally shadow gem-owned files.
// Upstream ships this check to every fork anyway and notes the re-scoping as an
// open maintainer decision, so we scope it here.
//
// The signal is `.al-folio-overrides.yml`, the ledger written by
// `al-folio upgrade overrides accept`. Its presence means this repo is a
// customized site whose overrides have been reviewed, and drift is tracked by
// `al-folio upgrade overrides audit` rather than by this check. Every other
// assertion in this file still runs -- they catch real breakage in a fork, such
// as a merge dropping `theme: al_folio_core` or a plugin from `_config.yml`.
const isCustomizedSite = exists(".al-folio-overrides.yml");
